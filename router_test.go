...
func TestMain(t *testing.T) {
	...
	sendRequestToEndpoint(pathRandom, 2)
	...
	expected := map[string][]string{
		...
		"application_error": {
			fmt.Sprintf(`application_error{code="%s",endpoint="%s",system="%s"} %v`, "500", pathError, config.System, 4),
		},
		"endpoint_accessed": {
			...
			fmt.Sprintf(`endpoint_accessed{endpoint="%s",system="%s"} %v`, pathRandom, config.System, 2),
		},
		"response_duration": {
			fmt.Sprintf(`response_duration_seconds_count{endpoint="%s",system="%s"} %v`, pathHome, config.System, 1),
			fmt.Sprintf(`response_duration_seconds_count{endpoint="%s",system="%s"} %v`, pathRandom, config.System, 2),
		},
	}
